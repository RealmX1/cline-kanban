...
import { sweepPendingTaskMessageInjectionsForAllWorkspaces } from "./pending-task-message-injection-startup-sweep";
...
export async function createRuntimeServer(deps: CreateRuntimeServerDependencies): Promise<RuntimeServer> {
	...
	// 不清扫就会永远停在 pending。不 await 阻塞启动——清扫是纠偏动作，不是服务可用性的前置条件。
	void sweepPendingTaskMessageInjectionsForAllWorkspaces();

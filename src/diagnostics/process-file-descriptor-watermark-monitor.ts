// 本进程 fd 占用量的自监控——任何 fd 泄漏的常设绊线。
//
// 由来：2026-08 那次故障的病根不是「fd 涨到了 10818」，而是**涨到 10818 却全程无人喊一声**：进程一路
// 无声地越过 Darwin 的 OPEN_MAX(10240)，直到所有 git 调用一起 EBADF 才以「work tree 无法验证」的形式
// 间接显形，排障只能从零重建因果链。当时那个泄漏源（node-pty 的 kqueue）已随依赖升级修复，但本模块
// 针对的是**那一整类**故障而非那一个 bug：fd 是全进程共享的稀缺资源，任何一处新增泄漏都会以同样的
// 方式——先无声累积，再让一个毫不相干的子系统首先报错——显形。这类回归靠单测抓不到，只能靠在线绊线。
//
